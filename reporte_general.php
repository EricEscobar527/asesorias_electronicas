...
function Header()
{
$inicio = $_POST['inicio_fecha'];
$DateTime = DateTime::createFromFormat('Y-m-d', $inicio);
$newInicio = $DateTime->format('d-m-Y');

$final = $_POST['final_fecha'];
$DateTime = DateTime::createFromFormat('Y-m-d', $final);
$newFinal = $DateTime->format('d-m-Y');
    ...
    $this->Ln(10);
    $this->Cell(100);
    $this->Cell(-136,17,'Reportes del: '.$newInicio.' al '.$newFinal,0,0,'C');
    $this->Ln(11);
...
$consulta2 = "SELECT count(matricula) AS total FROM `asesoria` WHERE fecha BETWEEN '$inicio' AND '$final'";
...
$consulta = "SELECT `matricula`, `nombre`, DATE_FORMAT(fecha, '%d/%m/%y') AS fecha, `tipo_asesoria`, `para`, `tema`, TIME_FORMAT(hora_inicio,'%l%:%i% %p') AS hora_inicio, TIME_FORMAT(hora_final,'%l%:%i% %p') AS hora_final, `genero` FROM `asesoria` WHERE fecha BETWEEN '$inicio' AND '$final'";
